...
from langflow.api import health_check_router, log_router, reload_router, router
...
def create_app():
    ...
    logger.debug("=" * 30 + " REGISTERED ROUTES " + "=" * 30)
    for route in app.routes:
        if hasattr(route, "methods") and hasattr(route, "path"):
            logger.debug(f"Path: {route.path}, Methods: {route.methods}, Name: {route.name}")
        elif hasattr(route, "path"):
            logger.debug(f"Mounted Path: {route.path}, Name: {route.name}")
        else:
            logger.debug(f"Other route type: {type(route)}")
    logger.debug("=" * 30 + " END REGISTERED ROUTES " + "=" * 30)
